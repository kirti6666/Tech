...
export interface InvoiceSnapshot {
  ...
  buyer: {
    ...
    pincode: string;
    country: string;
    /** Buyer's GSTIN for B2B supplies. Empty for consumers and exports. */
    gstin: string;
  };
  ...
  /** Zero-rated export of services. Drives the LUT declaration on the PDF. */
  isExport: boolean;
...
export function computeInvoiceSnapshot(
  ...
  // Digital orders carry `billing`; the legacy retail shape carried
  // `shippingAddress`. Both are read so invoices issued before the digital
  // migration still render from their stored order.
  const addr = order.billing ?? order.shippingAddress ?? {};
  const buyerStateCode = String(addr.stateCode || "").trim() || stateCodeFor(addr.state);
  const sellerStateCode = String(settings.seller.stateCode || "").trim();

  // Export of services. A supply to a recipient outside India is zero-rated
  // under LUT: no GST is charged and the invoice must say why. Checked
  // before the inter-state test, because an export is not an inter-state
  // supply — charging IGST on it would be wrong in the other direction.
  const buyerCountry = String(addr.country ?? "IN").trim().toUpperCase();
  const isExport =
    settings.tax.gstEnabled && buyerCountry !== "IN" && buyerCountry !== "INDIA";
  ...
  const gstEnabled = settings.tax.gstEnabled && !isExport;
  const inclusive = settings.tax.pricesIncludeTax;
  const defaultRate = gstEnabled ? Number(settings.tax.defaultGstRate) || 0 : 0;

  /**
   * Invoice lines = products + purchased services.
   *
   * Rebranding and deployment are separately taxable supplies the buyer paid
   * for, so they have to appear as their own lines with their own SAC code.
   * Folding them into the product price would understate the product and
   * misdescribe the service, and a buyer claiming input credit needs to see
   * them itemised.
   *
   * Digital lines have no quantity; it defaults to 1 throughout.
   */
  const productItems: any[] = Array.isArray(order.items) ? order.items : [];
  const addonItems: any[] = Array.isArray(order.addons)
    ? order.addons.map((addon: any) => ({
        title: addon.productTitle
          ? `${addon.label} — ${addon.productTitle}`
          : addon.label,
        price: addon.price,
        quantity: 1,
        sacCode: addon.sacCode ?? settings.tax.defaultHsnCode,
        gstRate: addon.gstRate ?? settings.tax.defaultGstRate,
      }))
    : [];
  const items: any[] = [...productItems, ...addonItems];

  const qtyOf = (it: any) => Number(it.quantity ?? 1) || 1;
  const grossTotal = round2(items.reduce((s, it) => s + it.price * qtyOf(it), 0));
  ...
  const lines: InvoiceLine[] = items.map((it, i) => {
    const gross = round2(it.price * qtyOf(it));
    ...
    // Per-line rate. Digital order items carry their own sacCode/gstRate
    // frozen at purchase time; retail items fall back to the global default.
    const rate = gstEnabled
      ? Number(it.gstRate ?? defaultRate) || 0
      : 0;
    ...
    return {
      ...
      variant,
      hsn: String(it.sacCode || settings.tax.defaultHsnCode || ""),
      quantity: qtyOf(it),
  ...
  const addressLines = [addr.addressLine1 ?? addr.line1, addr.addressLine2 ?? addr.line2]
    .filter(Boolean)
    .map(String);
  ...
  return {
    ...
    buyer: {
      // Digital orders store the payer as billing.name; retail orders stored
      // the recipient as shippingAddress.fullName.
      name: addr.name || addr.fullName || buyer.name || "",
      email: addr.email || buyer.email || "",
      ...
      gstin: String(addr.gstin ?? "").toUpperCase(),
    },
    ...
    order: {
      ...
      paymentMethod:
        order.gateway === "stripe"
          ? "Prepaid (Stripe)"
          : order.gateway === "razorpay" || order.paymentMethod === "razorpay"
            ? "Prepaid (Razorpay)"
            : "Prepaid",
    ...
    // Place of supply for an export is the recipient's country, not a state.
    placeOfSupply: isExport
      ? `${buyerCountry} (96)`
      : addr.state
        ? `${addr.state}${buyerStateCode ? ` (${buyerStateCode})` : ""}`
        : "",
    isInterState,
    isExport,
    exportDeclaration: isExport
      ? "Supply of services meant for export under bond or Letter of Undertaking without payment of integrated tax."
      : "",
 ...
 *
 * Two engines now produce a number: lib/pricing.ts decides what to charge,
 * and this file decides what the tax invoice says. They implement the same
 * rules, but they are separate code, and the invoice snapshot is immutable
 * once issued — so a divergence discovered later can only be fixed with a
 * credit note, not an edit.
 *
 * Call this immediately after computing a snapshot. A rupee of tolerance
 * covers the round-off line; anything larger is a real disagreement and
 * should be investigated before the invoice is delivered.
 */
export function reconcileWithCharged(snapshot: InvoiceSnapshot): {
  ok: boolean;
  difference: number;
} {
  const difference = round2(
    snapshot.totals.grandTotal - snapshot.totals.amountCharged
  );
  if (Math.abs(difference) > 1) {
    console.error(
      `[invoice] Invoice ${snapshot.invoiceNumber} totals ₹${snapshot.totals.grandTotal} but order ${snapshot.order.shortId} was charged ₹${snapshot.totals.amountCharged}.`
    );
